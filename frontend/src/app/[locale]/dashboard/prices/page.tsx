...
export default function PricesPage() {
    ...
    const runApply = async () => {
        ...
        try {
            ...
            notify.success("prices.applied"); setPreview(null); setAmount(""); setReason(""); if (book) await loadCurrent(book.id);
        } catch (error) {
    ...
    return <RequireRole roles={["super_admin", "admin", "branch_manager", "accountant"]}>
                ...
                <main className="space-y-5">
                    ...
                    {book && canMutate && <>
                        ...
                        <Section step="۳" title={t("prices.chooseBranches")} help={t("prices.chooseBranchesHelp")} done={scope === "all_branches" || !!selected.length}>
                            ...
                            {currentLoading ? <div className="mt-3 flex h-28 items-center justify-center gap-2 rounded-xl border border-dashed border-ink/10 text-xs text-ink/40"><RefreshCw className="h-4 w-4 animate-spin" />{t("prices.loadingPrices")}</div> : <div className="mt-3 grid gap-3 md:grid-cols-2">{rows.map((row) => { const ok = eligible(row); const checked = scope === "all_branches" ? ok : selected.includes(row.branch_id); return <button key={row.branch_id} disabled={scope === "all_branches" || !isAdmin || !ok} onClick={() => setSelected((old) => old.includes(row.branch_id) ? old.filter((id) => id !== row.branch_id) : [...old, row.branch_id])} className={cn("rounded-xl border p-4 text-start", checked ? "border-primary/30 bg-primary/[0.04] ring-2 ring-primary/[0.06]" : "border-ink/[0.08] bg-white", !ok && "opacity-45")}><div className="flex gap-3"><span className={cn("flex h-5 w-5 items-center justify-center rounded-md border", checked && "border-primary bg-primary text-white")}>{checked && <Check className="h-3 w-3" />}</span><div className="flex-1"><div className="flex justify-between"><b className="text-xs font-vazirmatn">{row.branch_name}</b><small className="text-ink/30">v{row.price_version}</small></div><div className="mt-3 flex justify-between"><span className="text-sm font-black text-primary">{formatNumber(Number(type === "selling_price" ? row.selling_price : row.consignment_cost || 0))} <small className="text-[8px] text-ink/35">{currencyLabel}</small></span><small className="text-[9px] text-ink/40">{t("prices.unsold")}: {formatNumber(row.unsold_qty)}</small></div>{row.reserved_qty > 0 && <p className="mt-2 text-[9px] text-amber-700">{t("prices.reserved")}: {formatNumber(row.reserved_qty)}</p>}</div></div></button>; })}</div>}
                        </Section>
